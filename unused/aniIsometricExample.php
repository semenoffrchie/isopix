 ...
 **/
include "classes/GifMerge.class.php";
...
$track->addSprite($trackSprite, array(-1, 1 ,2), array(0, 2 ,2), array(0, 3 ,2), array(2,6,0), array(2,7,0), array(3,8,0), array(3,9,0), array(4,10,0), array(4,11,0), array(5,12,0), array(5,13,0), array(6,14,0), array(6,15,0), array(7,16,0));
$isometricController->loadSpriteFromCollection($track);

$trackSprite = $isometricController->loadSpriteFromImage("sprites/slopetrack.gif", null, null, null, true);
$track = new spriteCollection;
$track->addSprite($trackSprite, array(1,4,1), array(1,5,0));
$isometricController->loadSpriteFromCollection($track);
/**
 * Render a train.
 * Co-ords:
 *   x: -1
 *   y: 1
 *   z: 50 (this makes it DEFO offscreen)
 **/
$train = $isometricController->loadSpriteFromImage("sprites/train.gif", -1, 1, 50);
 ...
 **/
$frames = array();
$coords = array(array(-1, 1 ,2), array(0, 2 ,2), array(0, 3 ,2), array(1,4,2), array(1,5,1), array(2,6,0), array(2,7,0), array(3,8,0), array(3,9,0), array(4,10,0), array(4,11,0), array(5,12,0), array(5,13,0), array(6,14,0), array(6,15,0), array(7,16,0));
for($frame=0;$frame<=count($coords);$frame++) {
	$isometricController->renderImage(-2, "frame" . $frame . ".gif");
	$isometricController->sprite[$train]->x = $coords[$frame][0];
	$isometricController->sprite[$train]->y = $coords[$frame][1];
	$isometricController->sprite[$train]->z = $coords[$frame][2];
	$frames[] = "frame" . $frame . ".gif";
}
$animation = new GifMerge($frames, 0, 0, 0, 0, 250, 0, 0, 'C_FILE');
print $animation->getAnimation();
?>